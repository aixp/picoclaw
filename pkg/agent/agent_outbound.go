...
func (al *AgentLoop) PublishResponseIfNeeded(ctx context.Context, channel, chatID, sessionKey, response string) {
	...
	if response == "Error processing message: hook aborted turn during before_llm: hook requested turn abort" {
		logger.DebugCF(
			"agent",
			"Skipped outbound (Error processing message: hook aborted turn during before_llm: hook requested turn abort)",
